...
namespace vefs
{
    vfile::vfile(vfilesystem *owner, detail::thread_pool &executor,
                 detail::file_id id, std::uint64_t maximumExtent,
                 inacessible_ctor)
        : mOwner(owner)
        , mId(id)
        , mFileTree()
    ...
    auto vfile::open_existing(vfilesystem *owner, detail::thread_pool &executor,
                              detail::archive_sector_allocator &allocator,
                              detail::file_id id, detail::sector_device &device,
    ...
    auto vfile::create_new(vfilesystem *owner, detail::thread_pool &executor,
                           detail::archive_sector_allocator &allocator,
                           detail::file_id id, detail::sector_device &device,
                           detail::file_crypto_ctx &cryptoCtx)
        -> result<std::shared_ptr<vfile>>
    ...
    {
        auto self =
            std::make_shared<vfile>(owner, executor, id, 0, inacessible_ctor{});
    ...
    retry:
        ...
        if (it < end)
        {
            ...
            {
                VEFS_TRY(
                    mFileTree->access_or_create(detail::tree_position(it)));
                mWriteFlag.mark();
        ...
        {
            if (!mMaximumExtent.compare_exchange_weak(
                    maximumExtent, size, std::memory_order_acq_rel,
                    std::memory_order_acquire))
            {
                goto retry;
            }
