...
public:
            ...
            -> result<void>
    {
        ...
        DPLX_TRY(encode(outStream,
                        static_cast<std::uint64_t>(value.data.root.sector)));
            ...
            -> result<void>
    {
        ...
        for (auto i = 0; i < 9; ++i)
        {
            ...
            case 1:
            {
                ...
                parsed |= 0x01;
                auto fileId = as_writable_bytes(std::span(value.fileId.data));
        ...
        -> unsigned int
{
    ...
    auto const idSize = encoded_size_of(1u)
                      + encoded_size_of(as_bytes(std::span(entry.fileId.data)));

    auto const pathSize = encoded_size_of(2u) + encoded_size_of(entry.filePath);

    auto const secretSize = encoded_size_of(3u) + encoded_size_of(entry.secret);

    auto const secretCounterSize
            = encoded_size_of(4u) + encoded_size_of(entry.secretCounter.view());

    auto const rootSectorSize = encoded_size_of(5u)
                              + encoded_size_of(static_cast<std::uint64_t>(
                                      entry.data.root.sector));
