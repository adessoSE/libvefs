        ...
        -> const error_domain &
{
    ...
    return *nonstandard_category_domain_map
                    .emplace(&cat, std::make_unique<std_adapter_domain>(&cat))
                    .first->second;
}
} // namespace

auto make_error(std::error_code ec, adl::disappointment::type) noexcept -> error
{
    static_assert(sizeof(int) <= sizeof(error_code));
    return {static_cast<error_code>(ec.value()), adapt_domain(ec.category())};
}
