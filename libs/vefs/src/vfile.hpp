...
namespace vefs
{
    ...
    public:
        vfile(vfilesystem *owner, detail::thread_pool &executor,
              detail::file_id id, std::uint64_t maximumExtent,
              inacessible_ctor);
        ~vfile();

        static auto
        open_existing(vfilesystem *owner, detail::thread_pool &executor,
        ...
        static auto
        create_new(vfilesystem *owner, detail::thread_pool &executor,
                   detail::archive_sector_allocator &allocator,
                   detail::file_id id, detail::sector_device &device,
                   detail::file_crypto_ctx &cryptoCtx)
            -> result<std::shared_ptr<vfile>>;
