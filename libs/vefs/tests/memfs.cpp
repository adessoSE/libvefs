...
namespace vefs::tests
{
    namespace
    {
        class memvefs_category_impl : public std::error_category
        {
        public:
            ...
            virtual std::string message(int errval) const override
            {
                ...
                {
                case memvefs_code::no_write_mode:
                    return "requested a file write on a file not opened for writing";
                case memvefs_code::no_read_mode:
                    return "requested a file read on a file not opend for reading";
                case memvefs_code::out_of_range:
                    return "tried to read past the end of the file";
                case memvefs_code::file_not_found:
                    return "the requested file wasn't found";
                case memvefs_code::out_of_memory:
                    return "could not grow the file, because not enough memory was available";
                default:
    ...
    void memory_file::read(rw_dynblob buffer, std::uint64_t readFilePos, std::error_code &ec)
    {
        if (!buffer)
        {
        ...
        mMemory->access(readFilePos, buffer.size(), [&buffer](auto dataChunk) {
            copy(dataChunk, buffer);
            buffer = buffer.subspan(dataChunk.size());
        });
    }

    void memory_file::write(ro_dynblob data, std::uint64_t writeFilePos, std::error_code &ec)
    {
        if (!data)
        {
            BOOST_THROW_EXCEPTION(logic_error{});
        ...
        mMemory->access(writeFilePos, data.size(), [&data](auto chunk) {
            copy(data, chunk);
    ...
    std::future<void> memory_file::read_async(rw_dynblob buffer, std::uint64_t readFilePos,
    ...
    std::future<void> memory_file::write_async(ro_dynblob data, std::uint64_t writeFilePos,
                                               file::async_callback_fn callback)
    {
    ...
    std::future<void> memory_file::resize_async(std::uint64_t newSize, file::async_callback_fn callback)
    {
