...
inline auto
system_allocator<ALIGNMENT>::allocate(const std::size_t size) noexcept
        -> allocation_result
{
    if (size == 0)
    {
...
template <std::size_t ALIGNMENT>
inline auto
system_allocator<ALIGNMENT>::reallocate(const memory_allocation memblock,
                                        const std::size_t newSize) noexcept
        -> allocation_result
{
    if (newSize == 0)
    ...
    if constexpr (over_aligned)
    {
        ptr = reinterpret_cast<std::byte *>(
                _aligned_realloc(memblock.raw(), newSize, alignment));
    }
    else
    {
        ptr = reinterpret_cast<std::byte *>(
                std::realloc(memblock.raw(), newSize));
    }
    if (!ptr)
    {
        return errc::not_enough_memory;
...
template <std::size_t ALIGNMENT>
inline void
system_allocator<ALIGNMENT>::deallocate(memory_allocation memblock) noexcept
{
    ...
    if constexpr (over_aligned)
    {
        ...
        if (auto merrno = posix_memalign(&tmp, alignment, size); merrno != 0)
        {
            ...
            return outcome::failure(
                    std::error_code{merrno, std::system_category()});
        }
        ...
        -> allocation_result
{
    ...
    if constexpr (over_aligned)
    {
        static_assert(!over_aligned, "posix doesn't specify a way to "
                                     "reallocate over aligned memory, sorry.");
    }
    else
    {
        ptr = reinterpret_cast<std::byte *>(
