...
namespace vefs::detail
{
    ...
    using fast_atomic_uint_with_bits =
        std::conditional_t<N_BITS <= std::numeric_limits<unsigned int>::digits, unsigned int,
                           std::size_t>;

    template <std::size_t MAX_VALUE>
    using fast_atomic_uint_for_maxval =
        std::conditional_t<MAX_VALUE <= std::numeric_limits<unsigned int>::max(), unsigned int,
                           std::size_t>;
