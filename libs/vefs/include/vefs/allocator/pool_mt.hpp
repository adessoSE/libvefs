...
private:
    ...
    static constexpr std::size_t alloc_block_size
            = num_elems * avals::adj_elem_size;

    using alloc_ctr_value_type = fast_atomic_uint_for_maxval<num_elems>;
    using alloc_ctr_atomic_type
            = atomic_resource_counter<alloc_ctr_value_type, num_elems>;

    constexpr static auto prealloc(block_allocator_t &src)
...
constexpr pool_allocator_mt<ELEM_SIZE, NUM_ELEMS, BlockAllocator, ALIGNMENT>::
        pool_allocator_mt()
    : block_allocator_t()
...
template <std::size_t ELEM_SIZE,
          ...
          typename BlockAllocator,
          std::size_t ALIGNMENT>
inline pool_allocator_mt<ELEM_SIZE, NUM_ELEMS, BlockAllocator, ALIGNMENT>::
        ~pool_allocator_mt()
{
...
template <std::size_t ELEM_SIZE,
          ...
          std::size_t ALIGNMENT>
constexpr auto
pool_allocator_mt<ELEM_SIZE, NUM_ELEMS, BlockAllocator, ALIGNMENT>::prealloc(
...
template <std::size_t ELEM_SIZE,
          ...
          typename BlockAllocator,
          std::size_t ALIGNMENT>
inline auto pool_allocator_mt<ELEM_SIZE, NUM_ELEMS, BlockAllocator, ALIGNMENT>::
        intr_allocate(const std::size_t size) noexcept
        -> result<std::tuple<memory_allocation, std::size_t>>
{
...
template <std::size_t ELEM_SIZE,
          ...
          std::size_t ALIGNMENT>
inline auto
pool_allocator_mt<ELEM_SIZE, NUM_ELEMS, BlockAllocator, ALIGNMENT>::allocate(
...
template <std::size_t ELEM_SIZE,
          ...
          std::size_t ALIGNMENT>
inline auto
pool_allocator_mt<ELEM_SIZE, NUM_ELEMS, BlockAllocator, ALIGNMENT>::reallocate(
...
template <std::size_t ELEM_SIZE,
          ...
          std::size_t ALIGNMENT>
inline void
pool_allocator_mt<ELEM_SIZE, NUM_ELEMS, BlockAllocator, ALIGNMENT>::deallocate(
