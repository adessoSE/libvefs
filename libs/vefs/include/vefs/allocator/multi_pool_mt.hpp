...
namespace vefs::detail
{
    ...
    private:
        ...
        using alloc_ctr_atomic_type =
            detail::atomic_resource_counter<alloc_ctr_value_type, num_elems_per_block>;
        ...
        static constexpr auto init_load_ctr(std::size_t i) noexcept -> alloc_ctr_atomic_type;

        static constexpr auto init_block(block_allocator_t &blockAllocator, std::size_t i)
            -> atomic_byte_ptr;
    ...
    public:
        ...
        inline auto reallocate(memory_allocation memory, const std::size_t newSize) noexcept
            -> allocation_result;
...
#if BOOST_COMP_MSVC < BOOST_VERSION_NUMBER(20, 0, 0)
    template <std::size_t ELEM_SIZE, std::size_t NUM_ELEMS_PER_BLOCK, std::size_t NUM_BLOCKS,
              typename BlockAllocator, std::size_t ALIGNMENT>
    inline multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
        ...
        , mBlocks{}
    {
        if constexpr (num_blocks > 0)
        {
            ...
            if (allocation_result mx = block_allocator_t::allocate(alloc_block_size);
                mx.has_error())
            {
            ...
            {
                std::atomic_init(&mBlocks.front(), mx.assume_value().writeable_bytes().data());
            }
    ...
    template <std::size_t ELEM_SIZE, std::size_t NUM_ELEMS_PER_BLOCK, std::size_t NUM_BLOCKS,
              typename BlockAllocator, std::size_t ALIGNMENT>
    constexpr auto
    multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
                            ALIGNMENT>::init_load_ctr(std::size_t i) noexcept
        -> alloc_ctr_atomic_type
    {
        return i == 0 ? alloc_ctr_atomic_type{resource_is_initialized} : alloc_ctr_atomic_type{};
    }
    ...
    constexpr auto
    multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
                            ALIGNMENT>::init_block(block_allocator_t &blockAllocator, std::size_t i)
        -> atomic_byte_ptr
    {
        if (i == 0)
        {
    ...
    template <std::size_t ELEM_SIZE, std::size_t NUM_ELEMS_PER_BLOCK, std::size_t NUM_BLOCKS,
              typename BlockAllocator, std::size_t ALIGNMENT>
    constexpr multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
                                      ALIGNMENT>::multi_pool_allocator_mt()
        : mLoadCtr{detail::resource_is_initialized}
    ...
    template <std::size_t ELEM_SIZE, std::size_t NUM_ELEMS_PER_BLOCK, std::size_t NUM_BLOCKS,
              typename BlockAllocator, std::size_t ALIGNMENT>
    inline auto multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
    ...
    template <std::size_t ELEM_SIZE, std::size_t NUM_ELEMS_PER_BLOCK, std::size_t NUM_BLOCKS,
              typename BlockAllocator, std::size_t ALIGNMENT>
    auto multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
                                 ALIGNMENT>::intr_allocate(const std::size_t size) noexcept
        -> result<std::tuple<memory_allocation, std::size_t>>
    {
        ...
        do
        {
            ...
            case resource_acquire_result::do_init:
                if (allocation_result nextBlock = block_allocator_t::allocate(alloc_block_size);
                    !nextBlock.has_error())
                {
                    auto &mem = nextBlock.assume_value();
                    mBlocks[i].store(mem.writeable_bytes().data(), std::memory_order_release);
    ...
    inline auto multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
        ...
        -> allocation_result
    {
        OUTCOME_TRY(alloc, intr_allocate(size));
    ...
    template <std::size_t ELEM_SIZE, std::size_t NUM_ELEMS_PER_BLOCK, std::size_t NUM_BLOCKS,
              typename BlockAllocator, std::size_t ALIGNMENT>
    inline bool multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
                                        ALIGNMENT>::owns(memory_allocation memory) noexcept
    {
        return memory.size() <= avals::elem_size &&
               std::get<0>(block_info_of(reinterpret_cast<std::byte *>(memory.raw()))) != -1;
    }
    ...
    template <std::size_t ELEM_SIZE, std::size_t NUM_ELEMS_PER_BLOCK, std::size_t NUM_BLOCKS,
              typename BlockAllocator, std::size_t ALIGNMENT>
    inline void multi_pool_allocator_mt<ELEM_SIZE, NUM_ELEMS_PER_BLOCK, NUM_BLOCKS, BlockAllocator,
                                        ALIGNMENT>::deallocate(memory_allocation memory) noexcept
    {
