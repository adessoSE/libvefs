...
public:
    ...
    // I ran into this issue, when using it in conjunction with
    // std::allocate_shared static_assert(alignof(T) <= impl_t::alignment);
    ...
    [[nodiscard]] inline value_type *allocate(std::size_t n)
    {
        ...
        if (allocation_result mblk = mAllocator->allocate(n * sizeof(T));
            mblk.has_value())
        {
    ...
    inline void deallocate(T *p, std::size_t n)
    {
        mAllocator->deallocate(
                {reinterpret_cast<std::byte *>(p), n * sizeof(T)});
    }
