...
template <typename T>
constexpr auto compressed_span_storage<T, dynamic_extent>::data() const noexcept
        -> T *
{
    return mData;
}
...
public:
    ...
    constexpr span(
            Container &cont,
            std::enable_if_t<
                    detail::is_span_compatible_container_v<Container,
                                                           element_type>,
    ...
    constexpr span(
            ...
            std::enable_if_t<
                    detail::is_span_compatible_container_v<const Container,
                                                           element_type>,
                    std::nullptr_t> = nullptr) noexcept(noexcept(std::data(cont))
                                                        || noexcept(std::size(
                                                                cont)))
...
span(std::basic_string_view<CharT, Traits> &) -> span<
    ...
    : storage{nullptr, 0}
{
    static_assert(extent == dynamic_extent || extent == 0,
                  "Only dynamic and zero sized spans are constructible from a "
                  "nullptr literal!");
}
template <typename T, std::size_t Extent>
...
template <typename T, std::size_t Extent>
inline constexpr auto span<T, Extent>::operator[](size_type pos) const noexcept
        -> reference
{
    return data()[pos];
}
...
template <class T,
          ...
          std::size_t Y = X>
requires std::is_assignable_v<U &, T &> constexpr auto
copy(std::span<T, X> source, std::span<U, Y> dest)
{
