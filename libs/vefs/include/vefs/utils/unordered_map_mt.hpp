...
{
    template <
        typename Key, typename T, typename Hash = hash::default_weak_std<Key>,
    ...
    using unordered_map_mt =
        libcuckoo::cuckoohash_map<Key, T, Hash, KeyEqual, Allocator,
                                  SLOT_PER_BUCKET>;
    ...
    using unordered_string_map_mt = libcuckoo::cuckoohash_map<
        std::string, T, utils::hash::default_weak_std<std::string_view>,
        std::equal_to<>, Allocator, SLOT_PER_BUCKET>;
} // namespace vefs::utils
