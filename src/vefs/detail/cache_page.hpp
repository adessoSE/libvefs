...
private:
    ...
    static constexpr state_type tombstone_bit = one
                                             << (state_traits::digits - 1);
    /**
     * indicates that the entry has been modified and needs synchronization
     * i.e. the entry is not available for replacement even though no active
     * references exist
     */
    ...
    /**
     * if this bit is set a second chance must be granted (regardless of the ref
     * count)
     */
     ...
     */
    static constexpr state_type ref_mask
            = ~(tombstone_bit | dirty_bit | second_chance_bit);
...
public:
     ...
     *
     * If successful the replacement will need to be completed by calling
     * finish_replace() or cancel_replace(). The replacement will fail if the
     * page is dirty, referenced or has the second chance bit set.
     */
    inline auto try_start_replace() noexcept
            -> enum_bitset<cache_replacement_result>;
    /**
     ...
     * *>|outcome<value_type *> ctor(void *) noexcept \returns A page handle if
     * successful or any error produced by the construction function.
     */
    template <typename Ctor>
    inline auto finish_replace(Ctor &&ctor) noexcept ->
            typename std::invoke_result_t<Ctor, void *>::template rebind<
                    cache_handle<T>>;
    /**
    ...
    /**
     * acquires a handle to the current page without setting the second chance
     * bit
     */
...
private:
    ...
    std::atomic<state_type> mEntryState{tombstone_bit};
...
inline cache_page<T>::~cache_page() noexcept
{
    ...
    {
        // #TODO maybe log something about open cache page references on
        // destruction
        std::terminate();
    }
...
template <typename T>
inline auto cache_page<T>::try_start_replace() noexcept
        -> enum_bitset<cache_replacement_result>
{
    state_type current = mEntryState.fetch_and(~second_chance_bit,
                                               std::memory_order_acq_rel);
    if (current & second_chance_bit)
    {
        // respect second chance
        return current & dirty_bit ? cache_replacement_result::second_chance
    ...
    do
    {
        ...
        if (!(current == 0
              || (current & tombstone_bit && !(current & dirty_bit))))
        {
            // notify the owner if this entry is not referenced and dirty but
            // not dead which usually is a good time to consider synchronizing
            // this entry
            return current == dirty_bit ? cache_replacement_result::dirty
                                        : cache_replacement_result::referenced;
        }

    } while (!mEntryState.compare_exchange_weak(current, dirty_tombstone,
                                                std::memory_order_acq_rel,
...
inline auto cache_page<T>::finish_replace(Ctor &&ctor) noexcept ->
        typename std::invoke_result_t<Ctor,
                                      void *>::template rebind<cache_handle<T>>
{
    static_assert(std::is_nothrow_invocable_v<Ctor, void *>);
    auto rx = std::invoke(std::forward<Ctor>(ctor),
                          static_cast<void *>(&mValueHolder));

    using result_type = typename std::invoke_result_t<
            Ctor, void *>::template rebind<cache_handle<T>>;
    ...
    return cache_handle<T>(std::move(rx).assume_value(),
...
inline bool cache_page<T>::try_purge(bool ownsLastReference) noexcept
{
    ...
    mEntryState.store(tombstone_bit | remainingReferences,
...
{
    return mEntryState.fetch_or(dirty_bit, std::memory_order_acq_rel)
         & dirty_bit;
}
template <typename T>
inline bool cache_page<T>::mark_clean() noexcept
{
    return !(mEntryState.fetch_and(~dirty_bit, std::memory_order_acq_rel)
             & dirty_bit);
}
...
{
    return !(mEntryState.fetch_add(1, std::memory_order_acq_rel)
             & tombstone_bit);
}
template <typename T>
