...
private:
    ...
    static constexpr auto data_node_size = sector_device::sector_payload_size;
    ...
    sector_tree_seq(sector_device &device,
...
template <typename... AllocatorCtorArgs>
inline auto
sector_tree_seq<TreeAllocator>::create_new(sector_device &device,
                                           file_crypto_ctx &cryptoCtx,
                                           AllocatorCtorArgs &&...args) noexcept
        -> result<std::unique_ptr<sector_tree_seq>>
{
...
template <typename TreeAllocator>
inline auto
sector_tree_seq<TreeAllocator>::extract_alloc_map(utils::bitset_overlay allocs)
        -> result<void>
{
...
template <typename TreeAllocator>
inline auto
sector_tree_seq<TreeAllocator>::collect_next_layer(utils::bitset_overlay allocs)
        -> result<void>
{
...
{
    // mDataBlocks[0] = {};
    mNodeInfos[0].construct(node_allocator_type{mTreeAllocator, sector_id{}},
                            true);
    mLoaded = 0;
        ...
        -> result<void>
{
    ...
    mLoaded += 1;
    mNodeInfos[layer].construct(node_allocator_type{mTreeAllocator, ref.sector},
                                false);
    return success();
}
        ...
        -> result<void>
{
    ...
    for (; i < mRootInfo.tree_depth && mCurrentPath.position(i) != 0; ++i)
    {
        ...
        if (std::any_of(block.begin(), block.end(), utils::is_non_null_byte))
        {
            return success();
        }
    ...
    for (; i > 0; --i)
    {
        ...
        auto &block = node_data(i);
        if (auto xbegin = std::next(block.begin(), serialized_reference_size);
...
template <typename TreeAllocator>
inline auto sector_tree_seq<TreeAllocator>::sync_to_device(int layer) noexcept
        -> result<void>
{
    ...
    if (auto writerx = mDevice.write_sector(updatedRef.mac, mCryptoCtx,
                                            writeSector, node_data_span(layer));
        !writerx)
    {
        mTreeAllocator.on_leak_detected();
