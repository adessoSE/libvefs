...
std::ostream &operator<<(std::ostream &str,
                         enum_bitset<cache_replacement_result> val)
{
...
BOOST_AUTO_TEST_CASE(finish_replace_for_replaceable_handle_returns_object)
{
    ...
    auto rx = page.finish_replace(
            [](void *p) noexcept -> result<cached_value *> {
...
BOOST_AUTO_TEST_CASE(
        try_start_replace_for_dirty_and_referenced_returns_referenced)
{
    cache_page<cached_value> page;
    page.try_start_replace();
...
BOOST_AUTO_TEST_CASE(try_purge_returns_false_if_dead)
{
    ...
    auto rx = page.finish_replace(
            [&destructor_called](void *p) noexcept
            -> result<non_trivially_destructable_value *> {
                return new (p)
                        non_trivially_destructable_value(destructor_called);
            });
